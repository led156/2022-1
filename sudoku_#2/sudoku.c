...
void *check_rows(void *arg)
{
    ...
    for (i = 0; i < 9; i++) {
        int check[10] = {};     /* 해당 행에 1~9까지의 숫자가 있는지 확인하는 배열. 숫자 n이 있다면 n번 인덱스의 값은 1, 아니라면 0이 된다. (0번 인덱스는 사용하지 않음) */
        for (j = 0; j < 9; j++) {
            check[sudoku[i][j]] = 1;    /* sudoku[i][j]의 값(숫자)를 check 배열의 인덱스로 하여 해당 값을 1로 바꾼다. */
        }
        
        for (j = 1; j < 10 && check[j]; j++);               /* check의 1~9번 인덱스의 값이 0일때까지 계속 반복. 만약 모두 1이라면 j는 10이 되어 종료됨. */
        j == 10 ? (valid[0][i] = 1) : (valid[0][i] = 0);    /* 만약 j가 10이라면(check가 모두 1이라면) valid[0][i]의 값을 1로, 아니라면 0으로 한다. */
    }
    pthread_exit(NULL);
}
...
void *check_columns(void *arg)
{
    ...
    for (i = 0; i < 9; i++) {
        int check[10] = {};     /* 해당 열에 1~9까지의 숫자가 있는지 확인하는 배열. 숫자 n이 있다면 n번 인덱스의 값은 1, 아니라면 0이 된다. (0번 인덱스는 사용하지 않음) */
        for (j = 0; j < 9; j++) {
            check[sudoku[j][i]] = 1;    /* sudoku[j][i]의 값(숫자)를 check 배열의 인덱스로 하여 해당 값을 1로 바꾼다. */
        }
        
        for (j = 1; j < 10 && check[j]; j++);               /* check의 1~9번 인덱스의 값이 0일때까지 계속 반복. 만약 모두 1이라면 j는 10이 되어 종료됨. */
        j == 10 ? (valid[1][i] = 1) : (valid[1][i] = 0);    /* 만약 j가 10이라면(check가 모두 1이라면) valid[1][i]의 값을 1로, 아니라면 0으로 한다. */
    }
    pthread_exit(NULL);
}
...
void *check_subgrid(void *arg)
{
    ...
    int q = k / 3;          /* 해당 서브그리드 인덱스를 3으로 나눈 몫 */
    int r = k % 3;          /* 해당 서브그리드 인덱스를 3으로 나누고 남은 나머지 */
    int check[10] = {};     /* 해당 서브그리드에 1~9까지의 숫자가 있는지 확인하는 배열. 숫자 n이 있다면 n번 인덱스의 값은 1, 아니라면 0이 된다. (0번 인덱스는 사용하지 않음) */
    
    for (i = (0 + 3*q); i < (3 + 3*q); i++) {
        for (j = (0 + 3*r); j < (3 + 3*r); j++) {
            check[sudoku[i][j]] = 1;    /* sudoku[j][i]의 값(숫자)를 check 배열의 인덱스로 하여 해당 값을 1로 바꾼다. */
        }
    }
    
    for (j = 1; j < 10 && check[j]; j++);               /* check의 1~9번 인덱스의 값이 0일때까지 계속 반복. 만약 모두 1이라면 j는 10이 되어 종료됨. */
    j == 10 ? (valid[2][k] = 1) : (valid[2][k] = 0);    /* 만약 j가 10이라면(check가 모두 1이라면) valid[2][k]의 값을 1로, 아니라면 0으로 한다. */
